...
function buildFallbackHtml(page) {
  const electionItems = page.electionSummaries.map((election) => `
          <li>
  ...
  const regionItems = page.regionLeaders.map((region) => `
          <li>
...
function replaceFallback(html, fallback) {
  ...
  return html.replace(/<div id="root">[\s\S]*?<\/div>\s*<script type="module"/, `<div id="root">${fallback}\n    </div>\n    <script type="module"`);
}
...
function moveTagAfter(html, tagName, attrName, attrValue, afterPattern) {
  ...
  return html.replace(tagPattern, "").replace(afterPattern, (anchor) => `${anchor}\n    ${tag}`);
}
