...
정확도 우선 운영 URL은 Vercel 배포에 연결된 `https://vote.gubiko.com/`입니다. 화면은 저장 캐시를 즉시 보여준 뒤 `api/latest.js` 서버리스 함수의 백그라운드 실시간 수집 결과가 오면 최신 공식 데이터로 교체합니다. GitHub Pages URL은 더 이상 운영 URL로 사용하지 않습니다.
...
정확한 온디맨드 갱신은 서버리스 API가 있는 배포에서 동작합니다. `api/latest.js`의 기본 요청은 접속 UX를 위해 저장 캐시를 즉시 반환하고, 브라우저의 자동 갱신과 `지금 갱신` 버튼은 `./api/latest?live=1`을 백그라운드로 호출해 선관위 `VCCP09` 응답을 새로 수집합니다.
