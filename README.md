...
- GitHub Actions가 생성하는 `data/latest.json` 기반 전국 개표 요약, SEO용 초기 HTML 요약, 브라우저 60초 자동 갱신
...
3. 결과를 `data/latest.json`에 저장합니다.
4. `scripts/prepare-seo.mjs`가 현재 데이터 기준으로 `index.html`의 메타 태그, JSON-LD, JS 없는 초기 개표 요약, `sitemap.xml`의 `lastmod`를 갱신합니다.
5. 변경이 있으면 자동 커밋합니다.
6. Pages UI는 같은 저장소의 `data/latest.json`을 읽어 화면을 갱신합니다.
...
UI는 React 18 ESM과 htm을 브라우저에서 직접 로드해 렌더링합니다. 별도 Vite 빌드 산출물 없이 GitHub Pages의 루트 배포 구조를 유지하기 위한 선택입니다. `index.html`은 검색엔진과 JS 비활성 환경을 위한 정적 개표 요약을 먼저 제공하고, 실제 대화형 화면 상태는 `app.js`의 React 컴포넌트가 `#root`를 대체해 관리합니다.
